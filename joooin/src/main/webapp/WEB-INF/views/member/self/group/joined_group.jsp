...
<script>
	$(document).ready(function(){		
		...
		$(document).on("click", ".groupDeleteBtn", function(){
			...
			$.ajax({
			    type: "POST",                           
			    url: "${pageContext.request.contextPath}/member/deleteGroup",
			    data: {"groupId": groupId},
			    success: function(not_login){
			    	if (not_login){
			    		location.href = "${pageContext.request.contextPath}/notLogin";
			    	} else {
			    		location.href = "${pageContext.request.contextPath}/member/self/group/joined_group";
			    	}
			    }
			});
		});
		...
		var column=[
            ...
            {"data": "delete", name:"退出社團" , "orderable":false }
           ];
						...
						<tbody>
							<c:forEach var="group" items="${groupList}">
								<tr>
		    						...
		    						<td class="groupBtn"><p data-placement="top" data-toggle="tooltip" title="退出社團"><button value="${group.groupId}" class="groupDeleteBtn btn btn-danger btn-xs" data-title="退出社團" data-toggle="modal"><img class="linkBtn" src="<c:url value='/resources/img/icon_delete.png' />"/></button></p></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
