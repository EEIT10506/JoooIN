...
<script>
	$(document).ready(function(){		
		...
		$(document).on("click", ".eventCancelBtn", function(){
			var eventId = $(this).val();
			
			$.ajax({
			    type: "POST",                           
			    url: "${pageContext.request.contextPath}/member/cancelEvent",
			    data: {"eventId": eventId},
			    success: function(not_login){
			    	if (not_login){
			    		location.href = "${pageContext.request.contextPath}/notLogin";
			    	} else {
			    		location.href = "${pageContext.request.contextPath}/member/self/event/request_event";
			    	}
			    }
			});
		});
		...
		var column=[
            ...
            {"data": "delete", name:"取消報名" , "orderable":false }
           ];
						...
						<tbody>
							<c:forEach var="event" items="${eventList}">
								<tr>
		    						...
		    						<td class="eventBtn"><p data-placement="top" data-toggle="tooltip" title="取消報名"><button value="${event.eventId}" class="eventCancelBtn btn btn-secondary btn-xs" data-title="取消報名" data-toggle="modal"><img class="linkBtn" src="<c:url value='/resources/img/icon_delete.png' />"/></button></p></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
