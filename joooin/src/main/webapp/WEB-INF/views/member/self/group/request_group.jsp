...
<script>
	$(document).ready(function(){		
		...
		$(document).on("click", ".groupCancelBtn", function(){
			var groupId = $(this).val();
			
			$.ajax({
			    type: "POST",                           
			    url: "${pageContext.request.contextPath}/member/cancelGroup",
			    data: {"groupId": groupId},
			    success: function(not_login){
			    	if (not_login){
			    		location.href = "${pageContext.request.contextPath}/notLogin";
			    	} else {
			    		location.href = "${pageContext.request.contextPath}/member/self/group/request_group";
			    	}
			    }
			});
		});
						...
						<tbody>
							<c:forEach var="group" items="${groupList}">
								<tr>
		    						...
		    						<td class="groupBtn"><p data-placement="top" data-toggle="tooltip" title="取消申請"><button value="${group.groupId}" class="groupCancelBtn btn btn-secondary btn-xs" data-title="取消申請" data-toggle="modal"><img class="linkBtn" src="<c:url value='/resources/img/icon_delete.png' />"/></button></p></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
