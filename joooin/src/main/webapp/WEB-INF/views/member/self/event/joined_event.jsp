...
<script>
	$(document).ready(function(){		
		...
		$(document).on("click", ".eventDeleteBtn", function(){
			...
			$.ajax({
			    type: "POST",                           
			    url: "${pageContext.request.contextPath}/member/deleteEvent",
			    data: {"eventId": eventId},
			    success: function(not_login){
			    	if (not_login){
			    		location.href = "${pageContext.request.contextPath}/notLogin";
			    	} else {
			    		location.href = "${pageContext.request.contextPath}/member/self/event/joined_event";
			    	}
			    }
			});
		});
		...
		var column=[
            ...
            {"data": "delete", name:"退出活動" , "orderable":false }
           ];
						...
						<tbody>
							<c:forEach var="event" items="${eventList}">
								<tr>
		    						...
		    						<td class="eventBtn"><p data-placement="top" data-toggle="tooltip" title="退出活動"><button value="${event.eventId}" class="eventDeleteBtn btn btn-danger btn-xs" data-title="退出活動" data-toggle="modal"><img class="linkBtn" src="<c:url value='/resources/img/icon_delete.png' />"/></button></p></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
